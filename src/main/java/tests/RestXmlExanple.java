...
public class RestXmlExanple {
	...
	public void testXMLResponse() {
		...
		//System.out.println(resp.xmlPath().getList("catalog.book.title.@id"));
		
		System.out.println("-------------------------------");

		System.out.println(resp.xmlPath().getList(
				"catalog.book.findAll{it.genre=='Fantasy'}"));
		
		System.out.println(resp.xmlPath().getList(
				"catalog.book.findAll{it.genre=='Fantasy'}.author"));
		
		System.out.println(resp.xmlPath().getList(
				"catalog.book.findAll{it.genre=='Fantasy'}.size()"));
	
		System.out.println(resp.xmlPath().getList(
				"catalog.book.findAll{it.genre=='Fantasy' | it.genre=='Romance'}.size()"));
	
		System.out.println(resp.xmlPath().getList(
				"catalog.book.findAll{it.genre=='Fantasy' | it.genre=='Romance'}.price"));
	
		
		System.out.println(resp.xmlPath().getList(
				"catalog.book.findAll{it.genre=='Fantasy' & it.genre=='Math'}.price"));
	
		
		System.out.println(resp.xmlPath().getString(
				"catalog.book.find{it.@id=='bk109'}.author"));
